...
download.file("https://ethz-ins.org/content/w11_practical.zip", "data/w11_practical.zip")
unzip("data/w11_practical.zip", exdir="data")
```

We have ATAC signal and 3 histone modifications as bigwig files (all limited to chr1),
but the DNA methylation data we have in GRanges format (methylation % for each CpG over all reads at this position):

```{r, load bigwigs}
DNAme <- readRDS("data/comparison/DNAme.rds")
tracks <- list(ATAC="data/comparison/ATAC.bw",
               H3K27ac="data/comparison/H3K27ac.bw",
               H3K4me3="data/comparison/H3K4me3.bw",
               DNAme=DNAme)
```
...
smp <- signal2Matrix(tracks, promoterRegions[1:2000], 
                     extend=1000, w=20, 
                     limit=c(0,100),
                     smooth=TRUE) # needed for methylation data as signal is very sparse
plotEnrichedHeatmaps(smp, 
                     axis_name="TSS",
                     multiScale=TRUE)
```

Shows exclusivity of accessible histone marks + ATAC signal and the DNA methlyation at the promoters.
Also shows the well-positioned +1 nucleosomes downstream of the TSS (H3K4me3 and H3K27ac). The ATAC signal is shifted to the left, as this shows us open chromatin which is exclusive with a histone mark.
...
With `background=NA` we see the raw data. By default, a value is imputed for the sites where there is not signal because there is no CpG site.


### TF binding sites

```{r}
bindingSites <- import.bed("data/comparison/CTCF_peaks.bed")
...
saveRDS(dmrRanges, "data/dmr.rds")
...
DMR.plot(dmrRanges, dmr=2, phen.col=rep(c("blue","darkred"), each=2), 
         arraytype="450K", # because of a bug in the newest version of DMRcate, we need to specify the array type here, even though we are working with BSseq data
         CpGs=bs, genome="hg19")
```
...
smDmrs <- signal2Matrix(DNAme_list, 
                        dmrRanges,
                        extend=1000, w=20, limit=c(0,100),
                        type="scale", smooth=TRUE)
colData(smDmrs)$group <- unlist(lapply(strsplit(names(DNAme_list), split="_"), "[", 1))
plotEnrichedHeatmaps(smDmrs, 
                     axis_name = c("dmr_start", "dmr_end"),
                     use_raster=TRUE)
# top_annotation=HeatmapAnnotation(Group=colData(smDmrs)$group)
```

This could then be used as input for `signal2Matrix()`.
